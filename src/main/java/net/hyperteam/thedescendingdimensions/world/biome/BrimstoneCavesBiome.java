...
public class BrimstoneCavesBiome extends TheDescendingDimensionsModElements.ModElement {
	@ObjectHolder("the_descending_dimensions:brimstone_caves")
	public static final CustomBiome biome = null;
	public BrimstoneCavesBiome(TheDescendingDimensionsModElements instance) {
		super(instance, 20);
	}
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0.5f).depth(0.1f).scale(0.2f).temperature(0.5f).precipitation(Biome.RainType.RAIN)
					...
					.surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(DitherrackBlock.block.getDefaultState(),
							DitherrackBlock.block.getDefaultState(), DitherrackBlock.block.getDefaultState())));
			setRegistryName("brimstone_caves");
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
		...
		protected boolean place(IWorldGenerationReader worldgen, Random rand, BlockPos position, Set<BlockPos> changedBlocks,
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == DitherrackBlock.block.getDefaultState().getBlock()
							|| ground == DitherrackBlock.block.getDefaultState().getBlock())
							&& (ground2 == DitherrackBlock.block.getDefaultState().getBlock()
									|| ground2 == DitherrackBlock.block.getDefaultState().getBlock())))
						return false;
					BlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						setTreeBlockState(changedBlocks, world, position.down(), DitherrackBlock.block.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == Blocks.AIR.getDefaultState().getBlock()
					|| blockType == Blocks.AIR.getDefaultState().getBlock() || blockType == DitherrackBlock.block.getDefaultState().getBlock()
					|| blockType == DitherrackBlock.block.getDefaultState().getBlock();
		}
